...
Nach der Wahl der Metadatenstandards (@sec-metadatenstandards-sieben-ebenen), müssen die Felder befüllt werden. Dazu kann für jedes Feld auf eine Reihe sogenannter **kontrollierter Vokabulare** beziehungsweise **Normdatensätze** (auch Normdateien genannt) zurückgegriffen werden. Hier kann zwischen Normdatensätzen (etwa der {{< glossary GND >}}) sowie kontrollierten Vokabularen und spezifischen Schlagwortindizes (etwa thematischen Indizes wie {{< glossary GenderOpen >}}) unterschieden werden. Für Ortsverzeichnisse wird oft auch {{< glossary Gazetteer >}} verwendet. Auch {{< glossary DCTerms >}} stellt mit den [**DCMI Metadata Terms**](https://www.dublincore.org/specifications/dublin-core/dcmi-terms/) übersichtliche Datenwertstandards mit einigen Dutzend Einträgen bereit. Diese sollten nicht mit dem {{< glossary DCMES >}} verwechselt werden.
...
- **Unsicherheit:** Unscharfe Angaben formal kennzeichnen (zum Beispiel {{< glossary EDTF >}} "\~1905").
- **Häufige Fehler:**
...
- Sprachspezifika berücksichtigen (zum Beispiel _Race_ ≠ _Rasse_)
...
KI-Modelle generieren im Projekt automatische Alt-Texte und verbessern so die Barrierefreiheit. Ohne systematische Qualitätskontrolle bleiben jedoch Risiken inkonsistenter oder diskriminierender Beschreibungen [@mahr2025y].

"Human-in-the-loop" ist kein Garant für Sicherheit: Fachpersonen benötigen Zeit, Schulung und klare {{< glossary Workflow display="Workflows" >}}. Definierte Rollen (zum Beispiel Kuratierung, Technik, Ethik-Board), nachvollziehbare Versionshistorien und Eskalationsverfahren sind notwendig, ebenso wie gezielte Trainings zu Bias-Erkennung und Prompt-Design.
:::
...
Es müssen konkrete Einsatzszenarien definiert werden (zum Beispiel automatische Transkription, Übersetzungen, Vorschläge für Schlagwörter) und es ist klarzustellen, wann ausschliesslich menschliche Expertise erforderlich ist (zum Beispiel Kontextualisierung oder Bewertung kontroverser Inhalte). Ergänzend können Ansätze der Explainable AI (xAI) und Metriken wie Fairness, {{< glossary Precision >}} und {{< glossary Recall >}} helfen, die Qualität systematisch zu evaluieren.
:::
